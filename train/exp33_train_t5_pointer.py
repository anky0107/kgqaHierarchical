"""
Exp 33: T5 Index-Pointer Reranker Training
=============================================

Fine-tunes FLAN-T5-base on the hardest 50 negatives.
Objective: Given a question and 50 candidate entities + paths,
the model must output the EXACT INDEX of the correct candidate, followed by its reasoning trace.
This structurally prevents string formatting hallucinations that degraded Exp 32.
"""
...
TRAIN_FILE = os.path.join(ROOT, "data/exp33_t5_pointer_train.json")
OUTPUT_DIR = os.path.join(ROOT, "checkpoints/exp33_t5_pointer")
FINAL_CKPT = os.path.join(ROOT, "checkpoints/exp33_t5_pointer_s3.pt")
...
def main():
    ...
    if not os.path.exists(TRAIN_FILE):
        raise FileNotFoundError(f"Training data not found: {TRAIN_FILE}. Run fast convert script first.")
    ...
    def preprocess_function(examples):
        ...
        # Tokenize targets
        # Increased to 128 to accommodate the reasoning trace
        labels = tokenizer(
            examples["target"],
            max_length=128,
            truncation=True,
    ...
    if not args.no_resume and os.path.isdir(OUTPUT_DIR):
        ...
        if ckpts:
            ...
            if os.path.exists(weight_file):
                ...
                print(f"[Exp33] Warm-resumed model weights from: {weight_file}")
                print(f"[Exp33] Optimizer state skipped intentionally to avoid VRAM paging.")
            else:
