"""
Exp 32: T5 Chain-of-Thought Reranker Training
=============================================

Fine-tunes FLAN-T5-base on the hardest 50 negatives.
Objective: Given a question and 50 candidate entities + paths,
the model must output a reasoning trace followed by the exact string of the correct entity.
"""
...
TRAIN_FILE = os.path.join(ROOT, "data/exp32_t5_cot_train.json")
OUTPUT_DIR = os.path.join(ROOT, "checkpoints/exp32_t5_cot")
FINAL_CKPT = os.path.join(ROOT, "checkpoints/exp32_t5_cot_s3.pt")
...
def main():
    ...
    if not os.path.exists(TRAIN_FILE):
        raise FileNotFoundError(f"Training data not found: {TRAIN_FILE}. Run exp32 dataset builder first.")
    ...
    if not args.no_resume and os.path.isdir(OUTPUT_DIR):
        ...
        if ckpts:
            ...
            if os.path.exists(weight_file):
                ...
                print(f"[Exp32] Warm-resumed model weights from: {weight_file}")
                print(f"[Exp32] Optimizer state skipped intentionally to avoid VRAM paging.")
            else:
