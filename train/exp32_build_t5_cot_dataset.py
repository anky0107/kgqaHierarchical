"""
Exp 32: Build T5 Chain-of-Thought (CoT) Dataset
========================================
...
Instead of predicting just the answer, the model must predict a reasoning trace.
...
def build_cot_prompt(question, candidates, global_path_str):
    ...
    return prompt, gold_name, gold_path_nl
...
def main():
    ...
    out_path   = os.path.join(ROOT, "data/exp32_t5_cot_train.json")
    ...
    for item in tqdm(data, desc="Building T5 CoT Prompts"):
        q = item["question"]
        ...
        # Build prompt
        prompt, gold_name, gold_path_nl = build_cot_prompt(q, s2_cands, path_str)
        
        if not gold_name:
            continue
            
        if gold_path_nl:
            target = f"Reasoning: The relations involved are {gold_path_nl}. Therefore the correct entity is {gold_name}. Answer: {gold_name}"
        else:
            target = f"Reasoning: This entity directly matches the question constraints. Answer: {gold_name}"
    ...
    print(f"\nSaving {len(cot_dataset)} CoT samples to {out_path}...")
