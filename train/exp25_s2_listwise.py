...
KL-Distillation Listwise Loss — the same loss that won the Stage 3 ablation.
...
ranker over the full candidate set, preventing gold-entity loss in the 200→15 cut.

Architecture: Same PathAwareRanker (MPNet-base-v2 + MLP fusion) as before.
Dataset:      exp18_cds_train_hard_full.json (27k hard negatives — 1 gold + ≤15 negs)
Loss:         KL-Distillation (gold soft-label = 1.0, neg soft-label = 0.1/(N-1))

Checkpoint: checkpoints/exp25_s2_listwise.pt
Metrics:    metrics/exp25_s2_listwise.csv
"""
...
METRICS_CSV  = "exp25_s2_listwise.csv"
...
class ListwiseS2Dataset(Dataset):
    def __init__(self, json_path: str, max_neg: int = 15):
        ...
        print(f"[Exp25 Dataset] {len(self.samples)} samples with gold labels "
...
class Exp25Trainer:
    def __init__(self, device: torch.device, max_neg: int = 15,
        ...
        print(f"[Exp25] Loaded PathAwareRanker ({ENCODER_NAME})  |  lr={lr}")
    ...
    def train(self, dataset: ListwiseS2Dataset, loader: DataLoader,
        ...
        print(f"\n[Exp25 S2] Training  |  epochs={epochs}  "
        ...
        for ep in range(start_epoch, epochs):
            ...
            ep_ckpt = os.path.join(ROOT, "checkpoints", f"exp25_s2_epoch{ep+1}.pt")
        ...
        print(f"[Exp25] Final checkpoint -> {final_ckpt}")
        return final_ckpt
...
def evaluate(model: PathAwareRanker, tok, dataset: ListwiseS2Dataset,
    ...
    for batch in tqdm(loader, desc="[Exp25] Isolated Eval"):
        item  = batch[0]
        q     = str(item["question"])
        path  = str(item.get("path") or "")
    ...
    print(f"[Exp25 Isolated Eval] Stage 2 Hit@1 = {hit1:.4f}  ({hits}/{total})")
    return hit1
...
def main():
    ...
    if not os.path.exists(train_file) and not args.eval_only:
        raise FileNotFoundError(
            f"\n[Exp25] Training data not found: {train_file}\n"
            "  Run `python train/exp18_hard_negative_mining.py` first.\n")

    trainer = Exp25Trainer(device)

    if args.eval_only:
        final_ckpt = os.path.join(ROOT, "checkpoints", CKPT_NAME)
        if not os.path.exists(final_ckpt):
            raise FileNotFoundError(f"[Exp25] Checkpoint not found: {final_ckpt}")
        ...
        if args.resume_epoch is not None:
            ep_ckpt = os.path.join(ROOT, "checkpoints",
                                   f"exp25_s2_epoch{args.resume_epoch}.pt")
            if os.path.exists(ep_ckpt):
                print(f"[Exp25] Resuming from epoch {args.resume_epoch}: {ep_ckpt}")
                trainer.model.load_state_dict(
                    torch.load(ep_ckpt, map_location=device))
                start_epoch = args.resume_epoch
            else:
                print(f"[Exp25] Epoch checkpoint not found; starting from scratch.")
    ...
    if os.path.exists(dev_file):
        ...
        print(f"[Exp25] Dev file not found ({dev_file}) — skipping isolated eval.")
