...
Evaluation Protocol:
  ...
  3. If predicted path matches gold â†’ answer is correct (Hits@1 = 1)
...
def predict_exp7_beam(model, tokenizer, question, device, valid_transitions, max_hops=4, beam_width=5):
    """Exp 7: returns per-hop top-k predictions using Graph-Constrained Beam Search."""
    enc = tokenizer(question, padding=True, truncation=True, max_length=128, return_tensors='pt')
    with torch.no_grad():
        with torch.amp.autocast('cuda'):
            out = model(enc['input_ids'].to(device), enc['attention_mask'].to(device))
    ...
    # [1, 4, num_rels] -> [4, num_rels]
    log_probs = F.log_softmax(rel_logits[0], dim=-1)
    
    # Beams: list of (path_of_r_ids, log_prob)
    beams = [([], 0.0)]
    
    for h in range(max_hops):
        new_beams = []
        hop_lp = log_probs[h]
        topk_lp, topk_idx = torch.topk(hop_lp, k=100, dim=-1)
        
        for path, score in beams:
            expanded = False
            for p, idx in zip(topk_lp, topk_idx):
                r = idx.item()
                # Apply hard constraints from training graph
                if len(path) > 0 and r not in valid_transitions.get(path[-1], set()):
                    continue
                new_beams.append((path + [r], score + p.item()))
                expanded = True
                
            # If all were invalid (dead end), just take the absolute argmax and pray
            if not expanded:
                best_r = topk_idx[0].item()
                best_p = topk_lp[0].item()
                new_beams.append((path + [best_r], score + best_p))
    ...
    for h in range(max_hops):
        top_ids = []
        for path, _ in beams:
            top_ids.append(path[h])
        results.append({'top_ids': top_ids})
...
def evaluate_model(samples, model, tokenizer, id2relation, device, 
    ...
    - If path matches â†’ answer is correct (standard planning evaluation)
    ...
    for sample in tqdm(samples, desc=f"Eval {model_name}"):
        ...
        elif model_type in ['beam']:
            # Exp 7 Beam Search
            hop_preds = predict_fn(model, tokenizer, question, device, max_hops=4, beam_width=5)
...
def main():
    ...
    # Build Valid Transitions Graph from Train Set
    print("\n  Building Transition Graph from Train Set...")
    valid_transitions = defaultdict(set)
    for sample in train_samples:
        path = sample['gold_path']
        for i in range(len(path) - 1):
            r1 = path[i]['relation_id']
            r2 = path[i+1]['relation_id']
            valid_transitions[r1].add(r2)
            
    print(f"    Built {len(valid_transitions)} transition rules.\n")
    
    # Evaluate Only on Dev and Test using Beam Search
    eval_datasets = [
        ("Dev", dev_samples),
        ("Test", test_samples)
    ]
    ...
    # --- Exp 7 (Beam Search) ---
    print("\n  ---- Exp 7: Scaled RoBERTa-Large (Global Beam Search) ----")
    ...
    predict_beam_fn = partial(predict_exp7_beam, valid_transitions=valid_transitions)
    
    for s_name, s_data in eval_datasets:
        r7 = evaluate_model(s_data, model, rob_tokenizer, id2rel_full, device, f"Exp 7-Beam ({s_name})", predict_beam_fn, model_type='beam')
        all_results.append(r7)
    del model; torch.cuda.empty_cache()
    
    # --- Write Results ---
    print("\n[3/3] Writing results...")
    rp = os.path.join(ROOT, 'results_beam.md')
    with open(rp, 'w', encoding='utf-8') as f:
        f.write("# KGQA Research Experiment Results\n\n")
        f.write(f"## End-to-End Evaluation\n\n")
        f.write("Evaluation protocol: question â†’ model predicts relation path â†’ path match against gold SPARQL â†’ derive answer correctness.\n")
